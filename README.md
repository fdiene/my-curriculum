...
- **api:** on the VPS, from the repo root: `docker compose --env-file ./.env -f infra/docker-compose.yml up -d --build` (joins the external `seo-prod-network` network; TLS via Let's Encrypt).
  - **`--env-file ./.env` is required**, not optional: Compose resolves `.env` relative to the directory the compose *file* lives in (`infra/`) by default, not the directory you run the command from. Without this flag, a `.env` placed at the repo root (matching `.env.example`'s location) is silently ignored - every var defaults to blank with a `WARN[...] The "X" variable is not set` for each one, and the container starts with no secrets at all rather than failing loudly. Do NOT use `--project-directory .` instead - that flag also changes how `build.context: ..` (`infra/docker-compose.yml`) resolves, breaking the build entirely (`resolve : lstat /root/apps: no such file or directory` when run from `~`, confirmed live on the seo-prod VPS). `--env-file` only affects `.env` lookup, leaving the build context untouched.
