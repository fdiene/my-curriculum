...
describe("scoreItem", () => {
  ...
  it("scores enterprise_architect by governance/security/integration weights", () => {
    expect(scoreItem(["security", "cloud", "api_design"] as any, "enterprise_architect")).toBe(26);
  });
});
...
describe("orderByRole", () => {
  ...
  it("puts governance/security-tagged items first for enterprise_architect", () => {
    const eaProjects = [
      { id: "seomnix", tags: ["ai_safety", "mlops", "dx_tooling"] as any, featured_for: [] as any },
      { id: "omnis", tags: ["plm", "cloud", "security", "aerospace"] as any, featured_for: [] as any },
    ];
    expect(orderByRole(eaProjects, "enterprise_architect").map((p) => p.id)[0]).toBe("omnis");
  });
});
