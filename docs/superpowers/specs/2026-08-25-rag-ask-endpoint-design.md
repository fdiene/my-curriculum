...
## Verified technical foundation (from a pre-planning spike, not assumed)

Before writing an implementation plan around unverified library claims, the local-embedding approach was actually spiked against this project's real Bun/Windows environment. Findings, since they materially change what the plan can safely assume:

- **Library: `@huggingface/transformers` (v4.2.0) with the `Xenova/all-MiniLM-L6-v2` model**, run via the `feature-extraction` pipeline with `{ pooling: "mean", normalize: true }`, produces real 384-dimension embeddings. Confirmed working end to end in this Bun 1.3.14/Windows setup.
- **Real gotcha: `onnxruntime-node` must be installed as an explicit direct dependency alongside `@huggingface/transformers`.** It is not auto-installed; omitting it fails with `ENOENT while resolving package 'onnxruntime-common'` the first time a pipeline is created. The plan must list both packages.
- **Use the quantized model variant** (`{ dtype: "q8" }` on the pipeline call): confirmed to produce the same 384-dim embeddings as the default fp32 model, at 22MB versus 87MB. This is the one to ship - fp32 offers no accuracy requirement this use case needs and quadruples the asset size for nothing.
- **The model file is not bundled by the npm/bun package itself - it is downloaded on first use and cached.** By default it caches under the library's own resolved install location (in this project's case, inside Bun's global package cache, not inside the repo or a per-project `node_modules`). `env.allowRemoteModels` defaults to `true`, meaning a cache miss triggers a live download from Hugging Face's CDN. For the production container this has a real consequence: the Docker build must explicitly pre-warm the model (run the pipeline once during image build, so the 22MB file is baked into an image layer) rather than let the first real visitor's request pay for a live download. The plan's Dockerfile changes must include this pre-warm step, and the running API should not depend on `allowRemoteModels` succeeding at request time.
...
- **The embedding model adds ~22MB to the API's Docker image** (quantized model, verified above) plus `onnxruntime-node`'s own native binary size, and holds the model in memory for the container's lifetime once loaded. Modest, but real - this is the one place this feature adds a genuinely new dependency to the project, despite the "no heavy dependencies" framing elsewhere. Unavoidable: *some* mechanism has to produce embeddings, and a hosted API was explicitly rejected in favor of this tradeoff.
- **First-request latency after a fresh container start** includes loading the ONNX model into memory (spiked at ~2.5s for the fp32 model; the smaller quantized model should be faster, but this should be confirmed during implementation, not assumed) - the pipeline should be initialized once at API startup, not per-request, so this cost is paid once per container lifetime, not once per visitor.
