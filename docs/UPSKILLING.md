...
- [x] Formatage des dates sur `/cv` (`YYYY-MM` brut -> « Jan 2020 » / « Janv. 2020 »). Priorité haute (verdict release review 2026-08-04). Approche tranchée : `Intl.DateTimeFormat` natif dans un petit helper de `@profile/core` (pas de date-fns/moment - respecte la règle zéro dépendance lourde), respectant le `lang` actif. Compétence : Intl API, formatage localisé sans dépendance. (fait 2026-08-04 : `packages/core/src/formatDate.ts` (`formatMonthYear`), sortie réelle vérifiée avant d'écrire les tests - "Jan 2020" EN, "janv. 2020" FR, "Jan. 2020" DE - branché sur experiences/education/certifications dans `CvDocument.vue`, 4 tests)
- [x] SEO `/cv` : passer `<meta name="robots">` de `index, follow` à `noindex` dans `cv.astro`. Verdict tranché (release review 2026-08-04) : `/cv` étant en `client:only="vue"`, Googlebot ne voit qu'une coquille vide - plutôt que lutter avec du SSR (casse l'architecture zéro-réseau), assumer que `/cv` est une route utilitaire (partage de lien direct, impression), pas une page d'atterrissage SEO ; `index.astro` reste l'unique page indexée. Pas d'ajout au sitemap.xml non plus. (fait 2026-08-04, vérifié dans le HTML buildé)
- [x] A11y des groupes de boutons (TemplateSwitcher/LangSwitcher dans `Terminal.vue` et `CvDocument.vue`) : ajouter `aria-label="Language selection"` / `aria-label="Template selection"` sur les conteneurs parents des switchers. Classé good-first-issue / nettoyage dette (release review 2026-08-04). (fait 2026-08-04 : `role="group"` ajouté en plus de `aria-label` - un `aria-label` seul sur un `<div>` sans rôle ARIA n'est pas fiablement annoncé par les lecteurs d'écran)
