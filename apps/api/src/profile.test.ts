...
describe("buildProfile", () => {
  ...
  it("orders ai_dx projects per spec (featured first, omnis last)", () => {
    ...
    expect(ids).toContain("artmap");
    expect(ids[ids.length - 1]).toBe("omnis");
  });
  ...
  it("puts OMNIS first for plm_architect (featured flagship)", () => {
    expect((buildProfile("plm_architect", "en", data).projects as any)[0].id).toBe("omnis");
  });
});
